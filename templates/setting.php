...
<div class="wrap nlmg">
    <h2>
        <img class="nlmg-logo" src="<?php echo $this->url ?>dist/img/icon-nlmg.png" align="Never Let Me go" height="32"
             width="32"/>
		<?php _e( 'Never Let Me Go setting', 'never-let-me-go' ); ?>
    ...
    <form method="post">
        ...
        <table class="form-table">
            <tbody>
            <tr>
                <th><label><?php _e( 'Allow user to self delete', 'never-let-me-go' ); ?></label>
                <td>
                    <label>
                        <input type="radio" name="nlmg_enable"
                               value="0"<?php checked( $this->option['enable'] == 0 ) ?> />
						<?php _e( 'Disabled', 'never-let-me-go' ); ?>
                    </label><br/>
                    <label>
                        <input type="radio" name="nlmg_enable"
                               value="1"<?php checked( $this->option['enable'] == 1 ) ?> />
						<?php _e( 'Enabled', 'never-let-me-go' ); ?>
                    </label>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <select id="nlmg_resign_page" name="nlmg_resign_page">
                        <option value="0"<?php selected( $this->option['resign_page'] == 0 ) ?>><?php _e( 'No resign page', 'never-let-me-go' ); ?></option>
						<?php
                    ...
                    <p class="description">
						<?php _e( 'Resign page means the static page which have form to resign.', 'never-let-me-go' ) ?>
                        <br/>
						<?php _e( 'If not specified, user can delete himself on profile page of admin panel.', 'never-let-me-go' ); ?>
                    </p>
                </td>
            </tr>
            <tr>
                <th><label><?php _e( 'Resign Way', 'never-let-me-go' ); ?></label>
                <td>
                    <label>
                    ...
                    <p class="description">
                        ...
                        <br/>
						<?php _e( 'To delete related information, see description below.<br />Please be careful with your country\'s low on other\'s privacy.', 'never-let-me-go' ); ?>
                    </p>
                </td>
            </tr>
            <tr>
    ...
    <p class="description">
		<?php _e( 'If you choose some resign page to publicly display, you can make show messag before resigning and after.', 'never-let-me-go' ); ?>
    </p>
    <ol class="nlmg-list">
        <li>
			<?php _e( 'Split assigned page\'s content with <code>&lt;!--nextpage--&gt;</code> tag.', 'never-let-me-go' ) ?>
        </li>
        <li>
			<?php _e( '1st page will be shown before resigning. Write some content for changing mind or inform your user about what to loose.', 'never-let-me-go' ); ?>
        </li>
        <li>
			<?php _e( 'User will be redirected to 2nd page after removal of their account. Write thank you message or something.', 'never-let-me-go' ); ?>
        </li>
    </ol>
    <p>
		<?php _e( 'Below is an example:', 'never-let-me-go' ); ?>
    </p>
    <pre class="nlmg-pre">
<?php _e( 'You are about to resign from our web magazine.', 'never-let-me-go' ); ?>

<?php _e( 'Are you sure to delete your account?', 'never-let-me-go' ); ?>

<?php _e( 'All of your data on our service will be deleted and can\'t be restored.', 'never-let-me-go' ); ?>

<span class="nlmg-tag">&lt;!--nextpage--&gt;</span>
<?php _e( 'Your account has been deleted successfully.', 'never-let-me-go' ); ?>

<?php _e( 'We miss you and hope to see you again.', 'never-let-me-go' ); ?>
</pre>
    <hr/>
    <h3><span class="dashicons dashicons-admin-tools"></span> <?php _e( 'How to customize' ) ?></h3>
    <ul class="nlmg-list">
        <li>
            <?php _e( 'Avoid user from leaving on specific condition.', 'never-let-me-go' ) ?>
        </li>
        <li>
			<?php _e( 'Change button labels.', 'never-let-me-go' ) ?>
        </li>
        <li>
			<?php _e( 'Do something if user leaves your site.', 'never-let-me-go' ) ?>
        </li>
    </ul>
    <p>
		<?php printf( __( 'There are some hooks available. For more details, please visit our support site <a href="%s" target="_blank">Gianism.info</a>', 'never-let-me-go' ), add_query_arg( array(
